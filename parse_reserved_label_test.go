...
// A module's name is part of the module path of every id below it, so this one
// is still refused — and says why.
func TestParseModuleNamedAfterAKeywordReportsWhy(t *testing.T) {
	p := setupParser(t)
...
`)
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "reserved names")
}
